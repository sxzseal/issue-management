...
const DEFAULTS = {
  prd: { maxSteps: 30, maxSubagents: 8, maxSelfCheckRetries: 1 },
  prototype: { maxSteps: 50, maxSubagents: 5, maxSelfCheckRetries: 0 },
  dev: { maxSteps: 100, maxSubagents: 30, maxSelfCheckRetries: 3 },
  // deploy is a standalone skill (not in session.phases); DEFAULTS used only
  // by read-only `check` calls — budget consume/set writes to session.phases
  // and will fail schema validation for deploy (by design).

...
// Session migration. Handles cold-start (missing shape) and v1→v2 upgrade
// (v2 adds phases.prd, drops phases.deploy — deploy is now an independent skill
// tracked via `lastDeploy` timestamp).
function migrateSession(data) {
  ...
  if (!migrated.currentPhase) migrated.currentPhase = 'prd';
  // v1→v2: promote prd to first-class phase, retire deploy
  if (!migrated.phases) {
    migrated.phases = {
      prd: { status: 'pending' },
    ...
    if (!migrated.phases.prd) migrated.phases.prd = { status: 'pending' };
    if (migrated.phases.deploy) {
      // Preserve deploy timestamp on the session root as lastDeploy for the
      // independent /dev-deploy skill, then drop the phase entry.
      const dep = migrated.phases.deploy;
      if (dep.completedAt && !migrated.lastDeploy) {
        migrated.lastDeploy = dep.completedAt;
      }
      delete migrated.phases.deploy;
    }
  }
  for (const k of ['prd', 'prototype', 'dev']) {
    if (!migrated.phases[k]) migrated.phases[k] = { status: 'pending' };
  }
  // Legacy currentPhase 'deploy' → coerce to 'done' (main pipeline now ends at dev)
  if (migrated.currentPhase === 'deploy') migrated.currentPhase = 'done';

...
- ✅ `.loop/acceptance-checklist.md` 存在（**主验收输入**，由 `/dev-proto` 生成）
- ✅ `.loop/prototype/stories-manifest.md` 存在（UI 契约）
- ✅ `.loop/prd.md` 存在（**推荐**：需求真源，dev-dev 会优先读作为消歧依据）

如果三者都不存在，**不要凭空开发**。停下来提示用户：

```
✗ 开发阶段需要 PRD / 验收清单 / 原型清单之一作为输入，现在 .loop/ 是空的。

请先选择：
  1. 跑 /dev-loop <需求> 完整管线（推荐）
  2. 跑 /dev-prd <需求> 生成 PRD，再 /dev-proto 生成原型
  3. 跑 /dev-proto <需求> --skip-prd 直接生成原型
  4. 手动放一份 .loop/acceptance-checklist.md（应急路径）
```
...
3. **重启**：开发被打断，现在恢复
4. **PRD 直连**：跑完 `/dev-prd` 后跳过原型（比如后端服务，UI 不重要）直接开发
...
2. 检查 `.loop/session.json`（用 `node scripts/lib/forge-state.mjs read .loop/session.json`）：
   - `currentPhase === "prd"` 且 PRD 未完成 → 提示用户「PRD 阶段未定稿，是否仍要进入开发？」
   - `currentPhase === "prototype"` 且原型未完成 → 提示用户「原型阶段未结束，是否仍要进入开发？」
   - `currentPhase === "done"` → 提示用户「开发已完成，重新进入会创建新的开发产物，是否继续？」
...
- `.loop/dev/checkpoint-findings.md`（Step 3 自检 findings）
- `.loop/session.json`（`currentPhase: "done"`）— 通过 `forge-state update --schema session` 写入

独立运行场景下**主管线在此结束**（部署不在主管线里），只输出：

```
✅ 开发阶段完成
   主管线已走完 (prd → proto → dev)
   下一步可选：
   - 部署：/dev-deploy [preview|staging|production]
   - 深度审查：/dev-review
   - 生成测试：/dev-test
```

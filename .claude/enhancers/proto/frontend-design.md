...
`frontend-design` skill 已由 ai-forge 框架内置，`scripts/create.sh` 会随项目一并落到 `.claude/skills/frontend-design/`，无需联网安装。若目录缺失（例如手动删除），运行 `scripts/upgrade.sh` 或从框架仓库重新拷贝即可恢复。

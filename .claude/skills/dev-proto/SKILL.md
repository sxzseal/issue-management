...
1. `.loop/prd.md`（**主输入**，dev-prd Phase 1 定稿产出；除非 `--skip-prd`）
2. 用户自然语言需求 `$ARGUMENTS`（PRD 存在时作为**补充说明**；`--skip-prd` 时作为主输入）
...
# 检查主输入：PRD（正常路径）+ 状态
ls .loop/prd.md 2>/dev/null && echo "PRD_EXISTS" || echo "NO_PRD"
node scripts/lib/forge-state.mjs read .loop/prd/state.json 2>/dev/null | head -10

# 检查是否有恢复上下文
cat .loop/acceptance-checklist.md 2>/dev/null | head -10

# 检查 currentPhase：若 prd 阶段未完成且未 --skip-prd，警告用户
node scripts/lib/forge-state.mjs read .loop/session.json 2>/dev/null | head -20
```

**PRD 状态判定（Step 0 关键分支）**：

- 有 `.loop/prd.md` 且 `session.json.phases.prd.status === "completed"` → **正常路径**，读 PRD 作为主输入
- 有 `.loop/prd.md` 但 `phases.prd.status !== "completed"` 且未 `--skip-prd` → 用 `AskUserQuestion` 提示"PRD 未定稿，建议先跑 `/dev-prd` 完成打磨，或加 `--skip-prd` 绕过"
- 无 `.loop/prd.md` 且未 `--skip-prd` → 用 `AskUserQuestion` 提示"没有 PRD，建议先跑 `/dev-prd` 或加 `--skip-prd` 用自然语言直接生成"
- 有 `.loop/prd.md` 且用户传了 `--skip-prd` → 明确忽略 PRD，走自然语言路径
...
**主分支：`.loop/prd.md` 存在 且未指定 `--skip-prd`**（正常从 `/dev-loop` 或 `/dev-prd` 接力过来的路径）：

- 读 PRD 作为主输入。跳过 4 维度澄清（PRD 已经打磨过了）
- 从 PRD 提取 Section 4 (用户故事) → stories 结构；Section 7 (API 契约) → MSW handlers；Section 8 (UI 规格) → theme + 布局
- `$ARGUMENTS` 若非空，作为**补充说明**（如"加个夜间模式" / "订单列表需要虚拟滚动"）叠加到 PRD 上
- 若 `$ARGUMENTS` 补充说明与 PRD 有实质冲突 → 用 `AskUserQuestion` 让用户裁决

**逃生分支：`--skip-prd` 或 `.loop/prd.md` 不存在**（快速验证 UI / 独立调用 / 冷启动路径）：

- 用户自然语言直接输入。需求清晰 → 直接进入下一步；需求模糊 → 用 `AskUserQuestion` 一次性问清 4 个维度（**只问真正不清楚的**，不照本宣科）：
    | 维度 | 示例问题 |
    |------|---------|

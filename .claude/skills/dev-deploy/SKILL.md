...
Step 6 完成时把 `selected` 数组落到本次部署的 checklist（`.loop/deploy/checklist.md` + `.loop/deploy/enhancers-manifest.md`），**不**写入 `session.json.phases.*`（dev-deploy 是独立 skill，不占用 phase 状态）：

```bash
# manifest 已在 Step 0 由 enhancers CLI 落盘，Step 6 无需重复写
# 若需要在部署报告里引用：
echo "本轮启用 provider: $(cat .loop/deploy/enhancers-manifest.md | grep '^- ')" >> .loop/deploy/checklist.md
```
...
对 Step 0 `enhancers select deploy` 返回的 `selected` 数组里的每个 provider（如 `["vercel", "railway"]`）：
...
**dev-deploy 是独立 skill**（跟 dev-review / dev-test 同类），**不**推进主管线 `currentPhase`，只记录 `lastDeploy` 时间戳 + 产物路径：
...
**部署 provider / 环境等元数据**落 `.loop/deploy/checklist.md`（人类可读）+ 部署 event log，不进 `session.json.phases.*`。

> Schema 校验会拒绝无效字段。CLI 用 read-modify-write + atomic rename，其他 phase 的字段不会被覆盖。
>
> 若某个 loop 需要"部署完就算 done"（比如冷启动直接部署的场景），用户可以手动跑 `/dev-loop --resume` 让主管线把 currentPhase 推到 done；dev-deploy 本身不做这个动作。
...
echo '{"artifacts":{"deployReport":".loop/deploy/checklist.md","deployMetrics":".loop/phases/deploy/metrics.json","loopSummary":".loop/loop-summary.json"}}' \

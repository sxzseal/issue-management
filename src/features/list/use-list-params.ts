...
const VALID_SORT: readonly SortField[] = [
  'created_at',
  'updated_at',
  'due_date',
  'priority',
...
export function useListParams(): [ListParams, ListParamActions] {
  ...
  const params = useMemo<ListParams>(() => {
    ...
    const due_from = /^\d{4}-\d{2}-\d{2}$/.test(dueFromRaw)
      ? dueFromRaw
      : undefined
    const due_to = /^\d{4}-\d{2}-\d{2}$/.test(dueToRaw) ? dueToRaw : undefined
    const q = sp.get('q') || undefined
    const sort = readClamped<SortField>(
      sp.get('sort'),
      VALID_SORT,
  ...
  const writePatch = useCallback(
    (patch: Partial<ListParams>) => {
      ...
      if ('status' in patch)
        write('status', (patch.status ?? []).join(',') || undefined)
      if ('priority' in patch)
        write('priority', (patch.priority ?? []).join(',') || undefined)
      if ('labels' in patch)
        write('labels', (patch.labels ?? []).join(',') || undefined)
      ...
      if ('pageSize' in patch && patch.pageSize) {
        write(
          'page_size',
          patch.pageSize !== DEFAULT_LIST_PARAMS.pageSize
